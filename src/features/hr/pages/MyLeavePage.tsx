...
export function MyLeavePage() {
  ...
  const list = useQuery({ queryKey: ["my-leave-requests"], queryFn: () => leaveApi.listMine({ limit: 200 }) });
  ...
  const submit = useMutation({
    mutationFn: ({ row, approverId }: { row: LeaveRequest; approverId: number }) =>
      leaveApi.submitMine(row.id, { title: `Leave — ${myName()} (${fmt(row.start_date)}–${fmt(row.end_date)})`, assigned_approver_user_id: approverId }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["my-leave-requests"] }); setPendingSubmit(null); toast.success("Submitted for approval", "Your selected approver has been notified by email."); },
  ...
  const remove = useMutation({
    mutationFn: (id: number) => leaveApi.removeMine(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["my-leave-requests"] }); toast.success("Deleted", "Leave request removed."); },
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <PageHeader title="My Leave" description="Request time off and track your approvals." />
      ...
      {list.isError && (
        <div className="p-5 rounded-2xl bg-rose-50 border border-rose-100 text-rose-600 flex items-center gap-3"><AlertCircle className="h-5 w-5" /><p className="text-sm font-bold">Failed to load your leave requests.</p></div>
      )}
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="bg-secondary-900/5 text-[10px] font-bold uppercase tracking-widest text-secondary-500">
              <th className="px-6 py-4">Period</th><th className="px-6 py-4">Leave days</th><th className="px-6 py-4">Reason</th><th className="px-6 py-4">Status</th><th className="px-6 py-4 text-right">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-secondary-100/60">
            {list.isLoading ? (
              ...
              <tr><td colSpan={5} className="px-6 py-24 text-center"><Calendar className="h-12 w-12 mx-auto text-secondary-200 mb-3" /><p className="font-bold text-secondary-900">No leave requests yet</p><p className="text-sm text-secondary-400 mt-1">Create one, then submit it for approval.</p></td></tr>
            ) : (
              requests.map((r) => (
                <tr key={r.id} className="hover:bg-primary-50/20">
                  <td className="px-6 py-4 font-bold text-secondary-900">{fmt(r.start_date)} → {fmt(r.end_date)}</td>
                  <td className="px-6 py-4 text-secondary-600">{String(r.days_requested || daysBetween(r.start_date, r.end_date))}</td>
                  <td className="px-6 py-4 text-secondary-500 max-w-[16rem] truncate">{r.reason || "—"}</td>
      ...
      <CreateMyLeaveModal isOpen={createOpen} onClose={() => setCreateOpen(false)} onCreated={() => qc.invalidateQueries({ queryKey: ["my-leave-requests"] })} />
      ...
      <SelectApproverModal
        ...
        summary={pendingSubmit ? `Leave ${fmt(pendingSubmit.start_date)} → ${fmt(pendingSubmit.end_date)}` : undefined}
...
function CreateMyLeaveModal({ isOpen, onClose, onCreated }: { isOpen: boolean; onClose: () => void; onCreated: () => void }) {
  const toast = useToast();
  ...
  // Default to the first available type once loaded (and whenever the modal opens).
  useEffect(() => {
  ...
  const create = useMutation({
    mutationFn: () => leaveApi.createMine({
  ...
  function resetForm() { setLeaveTypeId(""); setStart(""); setEnd(""); setReason(""); }
  function save() {
  ...
  return (
    <Modal isOpen={isOpen} onClose={() => { onClose(); resetForm(); }} title="New Leave Request" size="md"
      ...
      <div className="space-y-4">
        <Select
          ...
          hint={
            !types.isLoading && leaveTypes.length === 0
              ? "No leave types are configured yet. Ask HR to add them under HR → Leave types."
              : undefined
          }
        />
        ...
        <Textarea label="Reason (optional)" rows={2} value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Add any context for your approver…" />
      </div>
    </Modal>
  );
}
